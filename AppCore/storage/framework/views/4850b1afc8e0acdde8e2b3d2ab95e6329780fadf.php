<?php $__env->startSection('title'); ?>Alertas <?php $__env->stopSection(); ?>
                        ...
                        <div class="table-responsive b-0" data-pattern="priority-columns">
                            <datatable :columns="columns" :data="rows" :filter-by="filter" class="table table-bordered table-hover table-striped">
                                <template scope="{ row }" v-if="rows.length != 0">
                                    <tr :class="[row.days_end <= row.days_alert && row.days_end != 0  && row.status != 'Completada' ? 'tr-alert-warning' : '', row.days_end <= row.days_alert && row.days_end <= 0 && row.status != 'Completada' ? 'tr-alert-danger' : '']">
                        ...
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12">
                            <div class="row">
                                <div class="col-12 col-sm-12 col-md-6 col-lg-6">
                                    <strong>(*)</strong> <label for="description">Descripción: </label>
                                    <div class="form-group">
                                        <input type="text" class="form-control" name="description" placeholder="Descripción de la alerta" v-model="task.description">
                                    </div>
                                </div> 
                                <?php if(Auth::user()->is_admin): ?>
                                <div class="col-12 col-sm-12 col-md-6 col-lg-6">
                                    ...
                                    <div class="form-group">
                                        <select class="form-control" id="select2" name="residential" style="width: 100%;">
                                            <option value="" :selected="task.users_id == ''">-- Seleccionar --</option>
                                            <option v-for="res in residentials" :value="res.id" :selected="task.users_id == res.id">
                                                {{ res.name }} - {{ res.email }}
                                            </option>
                                        </select>
                                    </div>
                                ...
                                <?php else: ?>
                                <input type="hidden" name="residential" value="<?php echo e(Auth::user()->id); ?>">
                                <?php endif; ?>
                                <div class="col-12 col-sm-12 col-md-6 col-lg-6">
                                    <strong>(*)</strong> <label for="date_ini">Fecha de inicio: </label>
                                    <div class="form-group">
                                        <input type="date" class="form-control" name="date_ini" placeholder="Fecha de inicio de la alerta" v-model="task.date_ini">
                                    </div>
                                </div> 
                                <div class="col-12 col-sm-12 col-md-6 col-lg-6">
                                    <strong>(*)</strong> <label for="date_end">Fecha de vencimiento: </label>
                                    <div class="form-group">
                                        <input type="date" class="form-control" name="date_end" placeholder="Fecha de vencimiento de la alerta" v-model="task.date_end">
                                    </div>
                                </div>
                                <div class="col-12 col-sm-12 col-md-6 col-lg-6">
                                    <strong>(*)</strong> <label for="days_alert">Días de pre-aviso de la alerta: </label>
                                    <div class="form-group">
                                        <input type="number" min="0" max="365" class="form-control" name="days_alert" placeholder="Días por defecto donde se mostrará la alerta" v-model="task.days_alert">
                                    </div>
                                </div>
                                <div class="col-12 col-sm-12 col-md-6 col-lg-6">
                                    <strong>(*)</strong> <label for="status">Selecione Status</label>
                                    <div class="form-group">
                                        <select class="form-control" name="status" style="width: 100%;" v-model="task.status">
                        ...
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12">
                            ...
                            <div class="form-group">
                                <div class="checkbox icheck pl-20" >
                                    <label>
                                        <input type="checkbox" name="is_active" value="1" checked="checked" v-model="task.is_active" > Activar
                                    </label>
                                </div>
                            </div>
                        </div>
...
<script type="text/javascript" src="<?php echo e(asset('AppResources/plugins/momentjs/moment.js')); ?>"></script>
...
<?php if(Auth::user()->is_admin): ?> 
<script type="text/javascript" src="<?php echo e(asset('AppResources/js/admin/task/index.js')); ?>"></script>
<?php else: ?> <script type="text/javascript" src="<?php echo e(asset('AppResources/js/admin/task/indexClient.js')); ?>"></script> <?php endif; ?>
<?php $__env->stopPush(); ?>

<?php echo $__env->make('layouts.admin.main', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /opt/lampp/htdocs/AppSistemasIntegrados/AppCore/resources/views/admin/task/index.blade.php ENDPATH**/ ?>