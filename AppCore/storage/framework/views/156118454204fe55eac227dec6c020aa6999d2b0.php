			...
			<div class="box-body table-responsive">
				...
				<datatable :columns="columns" :data="rows" :filter-by="filter" class="table table-bordered table-striped">
					<template scope="{ row }">
						<tr>
							<td>{{ row.name }}</td>
							<td>{{ row.descripion }}</td>
							<td>{{ row.cantArticles }}</td>
							...
							<th class="text-right" style="width: 14%;">
								...
								<button type="button" class="btn btn-danger btn-sm " v-on:click="remove(row)" v-if="row.is_default == 0"><i class="fa fa-trash"></i></button>
							</th>
						</tr>
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.admin.main', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /opt/lampp/htdocs/AppGestor/AppGestorContenido/resources/views/admin/shop/categories.blade.php ENDPATH**/ ?>